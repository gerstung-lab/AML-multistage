...
#' Plot
#+ dev=c("png","pdf"), fig.width=4, fig.height=4, fig.args=list( pointsize = 8)
...
plot(survfit(surv[m] ~ TP53 + TPL, data=dataList), col=rep(set1[1:2],each=2), lty=1:2, mark=16)

#' ### Anova with CoxRFX
#' Prepating data
dataList <-list(G = data.frame(oncogenics), 
		T = cbind(makeInteger(clinicalData$Study[m]), TPL=clinicalData$TPL[m]),
		C = cbind(clinicalData[m, c("Age", "gender","BM.blasts","wbc","LDH_")], makeInteger(clinicalData$TypeAML[m])[,-1]))
dataList$GG <- makeInteractions(data.frame(oncogenics), data.frame(oncogenics))
dataList$GG <- dataList$GG[,-seq(1,ncol(dataList$GG), ncol(oncogenics)+1)] 
dataList$GG <- dataList$GG[,colSums(dataList$GG)>0] 
dataList$GT <- makeInteractions(dataList$G, dataList$T)
dataList$GT <- dataList$GT[,colSums(dataList$GT, na.rm=TRUE) > 0]

dataFrame <- do.call(cbind,dataList)

#' Fit model
#+ cache=TRUE
c <- coxRFX(dataFrame, surv[m], groups=unlist(sapply(names(dataList), function(x) rep(x, ncol(dataList[[x]])))))
...
d <- coxRFX(dataFrame[testIx,], surv[m][testIx], groups=unlist(sapply(names(dataList), function(x) rep(x, ncol(dataList[[x]])))))

p <- as.matrix(dataFrame[!testIx,]) %*% d$coefficients
