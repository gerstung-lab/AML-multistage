...
pWald <- pLR <- coefHat <- seeds <-  numeric(length(indeces))
for(i in seq_along(indeces)){
	...
	sim <- simulate(simDataFrame[1:x$nData,whichRFX], coxRFXFit, whichGene = as.character(x$gene), treatEffect = x$treatmentEffects, geneTreatEffect = x$interactionEffects, whichMain=whichMain)
	pWald[i] <- sim[1,1]
	pLR[i] <- sim[1,2]
	coefHat[i] <- sim[1,3]
}
simResults <- data.frame(pWald, pLR, coefHat, indeces, seeds)
